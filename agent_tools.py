...
@tool
def predict_insurance_bundle_tool(income: float, adult_dep: int, child_dep: int, vehicles: int, claims: int, user_name: str) -> str:
    """
    Use this tool ONLY when you have collected all the required profiling information from the user:
    income (float), adult_dep (int), child_dep (int), vehicles (int), claims (int), and user_name (str).
    It calls the ML model to predict the best insurance bundle and generates a personalized sales pitch.
    Returns the sales pitch you should deliver to the user.
    """
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            # We return the exact Imani explanation generated by the backend LLaMA-70B
            return data.get("imani_explanation", "Error generating pitch.")
        else:
            return f"❌ Error triggering ML prediction. Server returned {response.status_code}: {response.text}"
            
    except Exception as e:
        return f"❌ Error connecting to ML backend: {str(e)}"

@tool
def book_olea_appointment_tool(user_name: str) -> str:
    """
    Use this tool when the user agrees to buy or accepts the proposed insurance bundle.
    Requires user_name (str).
    Returns a confirmation message that a form was sent to OLEA to reserve an appointment.
    """
    return f"✅ Rendez-vous confirmé pour {user_name}. Un formulaire a été automatiquement envoyé aux services OLEA pour vous réserver le Pack et éviter toute bureaucratie !"
